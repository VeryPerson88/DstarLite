...
public class DStarLite implements java.io.Serializable{
	...
	{
		LinkedList<State> s = new LinkedList<State>();
		State tempState = new State(0,0, new Pair(-1.0,-1.0));
		...
		for(int i = -1; i<=1; i++){
			for (int j = -1; j<=1; j++){
				if (i != 0 || j != 0)
				tempState = new State(u.x + i, u.y + j, new Pair(-1.0,-1.0));
	...
	private boolean isValid(State u)
	{
		...
		if (cellHash.get(u).cost == -1) return false;
		return true;
	}
	...
	private boolean occupied(State u)
	{
		...
		if (cellHash.get(u) == null)
			return false;
		if (u.x > PixelGrid.gridSize || u.x < 0 || u.y > PixelGrid.gridSize || u.y < 0)
			return false;
	...
	public void main(String[] args)
	{
		...
		int[] src = {0,0};
		int[] dest = {0,0};
		for (int i = 0; i < PixelGrid.gridSize; i++) {
            for (int j = 0; j < PixelGrid.gridSize; j++) {
                if (PixelGrid.colors[j][i] == Color.GREEN) {
                    src[0] = i;
                    src[1] = j;
                }
                else if (PixelGrid.colors[j][i] == Color.ORANGE) {
                    dest[0] = i;
                    dest[1] = j;
                }
            }
        }
		pf.init(src[0], src[1], dest[0], dest[1]);
		for (int i = 0; i < PixelGrid.gridSize; i++) {
            for (int j = 0; j < PixelGrid.gridSize; j++) {
				if (PixelGrid.colors[j][i] == Color.GRAY){
					pf.updateCell(i, j, -1);
				}
			}
		}
		...
		for (State i : path)
		{
			PixelGrid.colors[i.y][i.x] = Color.BLUE;
		}
